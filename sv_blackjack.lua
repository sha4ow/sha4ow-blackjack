...
QBCore.Functions.CreateCallback('Shadow:server:ChipsAmount', function(source, cb)
    local retval = 0
...
QBCore.Functions.CreateCallback('Shadow-blackjack:canPlayMember', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
        local Membership = Player.Functions.GetItemByName("casino_Vip")
		 ...
		 if Chips ~= nil then
		 	if Chips.amount >= amount then
                ...
                TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['casinochips'], "remove", amount)
                 ...
                 TriggerClientEvent('blackjack:notify', source, 'You dont have enough chips', 'error')
                 return false
		 	end
		 else
             TriggerClientEvent('blackjack:notify', source, 'You dont have any chips', 'error')
		 	return false
		 end
        --TriggerClientEvent('blackjack:notify', source, 'Blackjack currently disabled as there is a bug.', 'error')
       -- return false
    end
end
...
function giveChips(source,amount)
    ...
    if Player ~= nil then
        if Player.Functions.AddItem('casinochips', amount, nil, {["quality"] = 100}) then
            ...
            TriggerClientEvent('blackjack:notify', source, 'You have to much in your pockets', 'error')
        end
    end
end
